...
private:
    ...
    public:
                                                             ...
                                                             : new GenericAudioProcessorEditor (*owner.getAudioProcessor()))
        {
            ...
            // EERIE - Mix Analyzer: the audio input is still muted by default (and
            // the "Mute audio input" toggle stays in the audio Settings dialog), but
            // we never show JUCE's "Audio input is muted to avoid feedback loop"
            // banner. Leaving shouldShowNotification false = no banner, no top gap.
            ignoreUnused (owner);
